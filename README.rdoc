...
<tt>gem 'comfortable_mexican_sofa', git: 'git@github.com:HitFox/comfortable-mexican-sofa.git'</tt>
...
Lastly, change the <tt>config.sign_out_via</tt> in <tt>devise.rb</tt> to <tt>:get</tt>.

Now if you want to create an admin-area controller using the +foxinator-generator+ you can:

<tt>rails g foxinator:create MyModel</tt>
...
=== The foxinator:create Command

When you create a new admin-area controller and model using the foxinator:create command there are a few additional steps to keep in mind.
